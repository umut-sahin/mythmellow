//! Implementation of the `bow-of-artemis` item.
...
pub fn attack(
    ...
    for (item_entity, &item_transform, &item_range) in item_query.iter() {
        ...
        let enemies_in_range = find_enemies_in_range_sorted_by_distance(
            &spatial_query,
            &item_position,
            &attack_area,
            &enemy_hit_box_query,
        ...
        for (_, enemy_position, enemy_distance) in enemies_in_range {
            ...
            let obstacle_between_item_and_enemy =
                find_obstacle(&spatial_query, &item_position, &enemy_direction, enemy_distance);

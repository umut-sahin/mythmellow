//! Resources of the `survival` mode.
...
pub struct SurvivalModeArguments {
    /// Wave to jump when starting directly in game.
    pub start_in_game_waves: Option<NonZeroU8>,
}

impl SurvivalModeArguments {
    /// Parses the arguments of the survival mode from the environment.
    ///
    /// # Native
    ///
    /// Arguments are parsed from the "mode" command line option.
    ///
    /// ```shell
    ...
    /// ```
    pub fn parse<'i>(args: impl Iterator<Item = &'i str>) -> SurvivalModeArguments {
        #[derive(Parser)]
        #[clap(name = "survival")]
        #[clap(disable_help_flag = true)]
        #[clap(disable_help_subcommand = true)]
        struct ArgsParser {
            /// Specifies the wave to jump when starting directly in game.
            #[arg(long)]
        ...
        let parser = ArgsParser::try_parse_from(args).unwrap_or_else(|error| {
            let full_error = format!("{}", error);
            let short_error = full_error.split('\n').next().unwrap();

            let error = short_error.trim_start_matches("error: ");
            log::error!(
                "unable to parse the arguments of the survival mode from ({})",
                error.replace("--", ""),
            );
        ...
        if !parser_display.is_empty() {
            log::info!("survival mode arguments:\n\n{}\n", parser_display.trim());
        }

        SurvivalModeArguments { start_in_game_waves: parser.wave }
    }
}
...
impl CurrentWave {
    ...
    /// Gets whether the current wave is the last wave.
...
/// Resource for the duration of the waves.
#[derive(Deref, Reflect, Resource)]
pub struct WaveDurations(pub Vec<Duration>);

impl WaveDurations {
    /// Gets the wave durations.
    pub fn get(waves: u8) -> WaveDurations {
...
impl Debug for WaveDurations {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let mut map = f.debug_map();
        for (i, duration) in self.iter().enumerate() {
            map.entry(&i, &duration);
        }
        map.finish()
    }
}

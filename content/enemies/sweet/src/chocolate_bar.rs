//! Implementation of the `chocolate-bar` enemy.
...
/// Color of the enemy.
pub const COLOR: Color = Color::srgba(0.32, 0.14, 0.03, 1.00);
...
pub const PROJECTILE_COLOR: Color = Color::srgba(0.22, 0.10, 0.03, 1.00);
...
pub fn attack(
    ...
    for (enemy_entity, enemy_transform) in enemy_query.iter() {
        ...
        let obstacle_between_enemy_and_player =
            find_obstacle(&spatial_query, &enemy_position, &player_direction, player_distance);
        if obstacle_between_enemy_and_player.is_some() {
            continue;
        }

        ProjectileBundle::builder()
            .originator(enemy_entity)

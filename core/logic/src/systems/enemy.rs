...
pub fn initialize_enemy_counter(mut commands: Commands) {
    log::info!("initializing the enemy counter");
...
pub fn initialize_enemy_spawn_pattern(world: &mut World) {
    ...
    let spawn_pattern =
        enemy_registry[enemy_pack_index.0].pack.spawn_pattern(world).unwrap_or_else(|| {
            let game_mode_registry = world.resource::<GameModeRegistry>();
            let game_mode_index = world.resource::<GameModeIndex>();
            game_mode_registry[game_mode_index.0].default_enemy_spawn_pattern(world)
        });

    log::info!("enemy spawn pattern: {:#?}", spawn_pattern);
    world.insert_resource(spawn_pattern);
}


/// Spawns enemies according to the enemy spawn pattern.
pub fn spawn_enemies(world: &mut World) {
    ...
    for (index, spawn) in spawns.iter_mut().enumerate() {
        if !spawn.delay.finished() {
            spawn.delay.tick(time.delta());
            if spawn.delay.just_finished() {
                spawn.remaining += spawn.count;
                log::info!(
                    "enemy spawn pattern {} is being applied for the first time \
                    ({:?} elapsed)",
                    index,
                    spawn.delay.duration(),
                );
        ...
        if let Some(repeat) = &mut spawn.repeat {
            ...
            if repeat.finished() {
                ...
                log::info!(
                    "enemy spawn pattern {} is being applied again \
...
pub fn spawn_enemy(world: &mut World, map_bounds: &MapBounds, spawn: &mut EnemySpawn) {
            ...
            _ => {
                ...
                let enemy_direction = {
                    ...
                    let rotation = if from_degrees == to_degrees {
                        ...
                        let mut rng = world.resource_mut::<GlobalEntropy<WyRand>>();
    ...
    if let Some(transform) = found_enemy_transform {
        log::info!("spawning the enemy {:?} at {}", enemy.id(), transform.translation.xy());
...
/// Finds free space to spawn an enemy.

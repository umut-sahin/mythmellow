...
/// Resource for the player registry.
...
impl PlayerRegistry {
    ...
    pub fn find_player(&self, player_id: impl AsRef<str>) -> Option<(usize, usize)> {
        ...
        for (mythology_index, entry) in self.iter().enumerate() {
            for (player_index, player) in entry.players.iter().enumerate() {
                if player.id() == player_id {
                    return Some((mythology_index, player_index));
                }
            }
        }
        None
...
pub struct PlayerRegistryEntry {
    /// Registered mythology.
    pub mythology: RegisteredMythology,
    /// Registered players in the mythology.
...
/// Container for the registered mythologies.
#[derive(Debug)]
...
/// Container for the registered players.
#[derive(Debug)]
